...
| F-15 | Spreminjanje statusa pobude | Delno implementirano | `updateInitiativeStatus()`, `src/main.js`, `supabase/schema.sql` | Rocni zagon aplikacije | UI omogoca spremembo statusa; produkcijsko mora biti omejeno na administratorje. |
...
| F-28 | Domenski testi | Implementirano | `tests/domain.test.mjs` | `npm test` | Pokrivajo validacijo, AI presojo, glasovanje, podpis, komentarje, analitiko, email domensko logiko in Turnstile server-side preverjanje. |
